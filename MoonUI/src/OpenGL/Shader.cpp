...
{
	glUniform1f(GetUnformLocation(name), value);
}
...
void Shader::SetUniform1iv(const std::string& name, int count, const int* v)
{
	glUniform1iv(GetUnformLocation(name), count, v);
}

void Shader::SetUniformMat4f(const std::string& name, const glm::mat4& matrix)
{
	glUniformMatrix4fv(GetUnformLocation(name), 1, false, &matrix[0][0]);
}
